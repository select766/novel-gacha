...
### LAN公開モード（他のホストからアクセス可能）

```bash
npm run dev:lan
```

フロントエンドが `0.0.0.0` にバインドされ、LAN上の他のホストからもアクセスできます。
起動時にターミナルに表示される `Network:` のアドレス（例: `http://192.168.x.x:5173`）でアクセスしてください。

> **注意:** LAN公開モードはローカルネットワーク内の全デバイスからアクセス可能になります。信頼できるネットワーク環境でのみ使用してください。
